...
const DocItem = ({
  ...
  // Check if data exists in DB (to lock the field)
  ...
  const handleView = () => {
    ...
    if (isUploadedLocally) {
      const file = form[`${prefix}_file`];
      url = URL.createObjectURL(file);
      isPdf = file.type === "application/pdf";
    } else {
      const dbUrl = form[`${prefix}_db_url`];
      url = `${IMAGE_BASE_URL}${dbUrl}`;
      isPdf = dbUrl.toLowerCase().endsWith(".pdf");
    }
      ...
      <div className="card-body p-3">
        <div className="d-flex align-items-center">
          ...
          <div className="flex-grow-1 overflow-hidden">
            ...
            <div
              className={`d-flex align-items-center border-bottom pb-1 ${
                // If it is locked (existing), show grey border, else show blue when typing
                isExistingInDb
            ...
            >
              <input
                type="text"
                className={`form-control border-0 p-0 shadow-none bg-transparent fw-bold ${
                  isExistingInDb ? "text-muted" : "text-dark"
                }`}
                style={{ fontSize: "0.95rem", letterSpacing: "0.5px" }}
                placeholder={placeholder}
                value={value}
                // LOGIC: If it exists in DB, it is Read Only. If not, you can type.
                readOnly={isExistingInDb}
                maxLength={
              ...
              />
              {/* REMOVED THE EDIT PEN BUTTON HERE */}
            </div>
            ...
            {hasExpiry && (
              <div className="mt-2 d-flex align-items-center">
                ...
                <div
                  className={`badge bg-danger bg-opacity-10 text-danger border border-danger px-2 py-1 position-relative ${
                    !isExistingInDb ? "cursor-pointer" : ""
                  }`}
                  // Only allow date picker click if NOT locked
                  onClick={() => !isExistingInDb && dateRef.current.showPicker()}
                >
                  ...
                  <input
                    ...
                    readOnly={isExistingInDb} // Lock date if existing
                    onChange={(e) =>
          ...
          <div className="d-flex flex-column gap-2 ms-2">
            {hasAnyDoc ? (
              // --- IF DOC EXISTS: SHOW VIEW BUTTON ---
              <button
        ...
        if (data.success) {
          ...
          data.documents.forEach((doc) => {
            ...
            if (p) {
              ...
              if (p === "dl") {
                 updatedForm.dl_expiry = doc.idnt_expdt || ""; 
                }
            }
          });
          setForm(updatedForm);
