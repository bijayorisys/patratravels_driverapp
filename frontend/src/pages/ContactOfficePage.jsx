  ...
  if (loading) {
    return (
      <div className="min-vh-100 bg-light d-flex flex-column align-items-center justify-content-center">
        ...
        <p className="text-muted fw-semibold animate-pulse">Loading Support Hub...</p>
        <style>{`
      ...
      <div className="px-3" style={{ maxWidth: "600px", margin: "0 auto" }}>
        ...
        <div className="card border-0 shadow-lg rounded-4 p-3 mb-4 bg-white">
          ...
          <div className="row g-2">
            ...
            <div className="col-6">
              <div 
                onClick={() => openLink(`tel:${contactData.primary_support?.phone}`)}
              ...
              >
                ...
                <span className="fw-bold text-dark small d-block">Call Support</span>
                <span className="text-muted extra-small">{contactData.primary_support?.phone}</span>
              </div>
            </div>
            ...
            <div className="col-6">
              <div 
                onClick={() => openLink(`mailto:${contactData.primary_support?.email}`)}
                className="p-3 rounded-4 bg-light text-center border cursor-pointer h-100 d-flex flex-column align-items-center justify-content-center hover-action"
              >
                <div className="bg-white p-2 rounded-circle shadow-sm mb-2 text-danger">
                  <Mail size={20} />
                </div>
                <span className="fw-bold text-dark small d-block">Email Ops</span>
                <span className="text-muted extra-small text-break" style={{ fontSize: "10px" }}>
        ...
        <div className="d-flex align-items-center justify-content-between mb-3 px-1">
          <h6 className="fw-bold text-secondary mb-0 text-uppercase small ls-1">Visit Our Hubs</h6>
        ...
        <div className="d-flex flex-column gap-3">
          {contactData.offices?.map((loc) => (
            <div key={loc.id} className="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
              <div className="card-body p-0">
                <div className="p-3">
                  <div className="d-flex justify-content-between align-items-start mb-2">
                    <h6 className="fw-bold text-dark mb-0 d-flex align-items-center gap-2">
                      {loc.color === "primary" ? <Building2 size={18} className="text-primary"/> : <MapIcon size={18} className="text-secondary"/>}
                      {loc.name}
                    </h6>
                    <span className={`badge bg-${loc.color} bg-opacity-10 text-${loc.color} rounded-pill border border-${loc.color} border-opacity-10`}>
                      {loc.type}
                    </span>
                  </div>
                  
                  <p className="text-muted small mb-3 ps-1" style={{ fontSize: "0.85rem", lineHeight: "1.5" }}>
                    {loc.address}
                  </p>
