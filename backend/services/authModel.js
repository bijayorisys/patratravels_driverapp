 ...
 */
export const requestOtp = async ({ driverRegNo, mobileNumber }) => {
  if (!driverRegNo || !mobileNumber) {
    throw new Error("Driver Registration Number and Mobile Number are required.");
  }
 ...
 */
export const setupMpin = async ({ mobileNumber, mpin, driverId }) => {
  console.log("setupMpin Params:", { mobileNumber, mpin, driverId });
  ...
  const [result] = await db.execute(
    "UPDATE tbl_drivers SET mpin_hash = ?, is_mpin_set = 1 WHERE driver_phno = ? AND driver_id = ?",
    [hashedMpin, mobileNumber, driverId]
  );
 ...
 */
export const resetMpin = async ({ mobileNumber, newMpin, driverId }) => {
  if (!mobileNumber || !newMpin || newMpin.length !== 4 || !driverId) {
  ...
  try {
    ...
    const [rows] = await db.pool.execute(
      `SELECT 
                ...
                t1.idnt_no, 
                t1.idnt_copy, 
                DATE_FORMAT(t1.idnt_expdt, '%Y-%m-%d') as idnt_expdt,
