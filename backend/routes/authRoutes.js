...
const asyncWrapper = (logicFunction) => async (req, res) => {
  try {
    ...
    const driverId = req.driver ? req.driver.id : null;

    const result = await logicFunction({ ...req.body, ip, driverId });
    ...
    try {
      ...
      if (req.files) {
        ...
        await Promise.all(fileKeys.map(async (key) => {
          ...
          const filename = `${key.replace('_file', '')}-${driverId}-${dateStr}-${timeStr}${extension}`;
          const filePath = path.join(docDir, filename);

          // 3. SAVE FILE
          if (isPdf) {
            // IF PDF: Save buffer directly (Sharp cannot process PDFs)
...
router.post("/setup-mpin", auth, asyncWrapper(setupMpin));
router.post("/reset-mpin", auth, asyncWrapper(resetMpin));
